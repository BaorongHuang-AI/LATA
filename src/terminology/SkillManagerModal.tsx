...
const SkillManagerModal: React.FC<Props> = ({ open, onClose, onSkillsChanged }) => {
  ...
  const [selected, setSelected] = useState<TerminologySkill | null>(null);
  ...
  const startEdit = (skill: TerminologySkill) => {
    setSelected(skill);
    setForm({
      name: skill.label,
  ...
  const handleSave = async () => {
    ...
    try {
      if (selected) {
        await window.api.updateTerminologySkill(selected.id, payload);
        message.success("Skill updated.");
      } else {
        await window.api.saveTerminologySkill(payload);
  ...
  const handleDelete = async () => {
    ...
    Modal.confirm({
      ...
      onOk: async () => {
        try {
          await window.api.deleteTerminologySkill(selected.id);
  ...
  return (
    <Modal
      title="Manage Extraction Skills"
      open={open}
      onCancel={onClose}
      footer={null}
      width={800}
          ...
          <div className="space-y-3">
            <div>
              ...
              <Input
                ...
                placeholder="e.g., Bilingual Terminology Extraction"
              />
            </div>
            ...
            <div>
              ...
              <Input.TextArea
                ...
                placeholder="You are a professional bilingual terminology extraction engine..."
              />
            </div>
            ...
            <div>
              ...
              <p className="text-xs text-gray-400 mt-1">
                ...
                corpus data. For custom extraction skills, also use{" "}
